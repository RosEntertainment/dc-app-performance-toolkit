...
def app_specific_action(webdriver, datasets):
    ...
    def measure():
        ...
        @print_timing("selenium_app_custom_action:view_macro_in_page")
        def sub_measure():
            page.go_to_url(f"{CONFLUENCE_SETTINGS.server_url}/pages/viewpage.action?pageId={app_specific_page_id}")
            page.wait_until_visible((By.ID, "title-text"))  # Wait for title field visible
        sub_measure()

        @print_timing("selenium_app_custom_action:view_user_dashboard")
        def sub_measure():
            page.go_to_url(f"{CONFLUENCE_SETTINGS.server_url}/#all-updates")
            page.wait_until_visible((By.CLASS_NAME, "list-title"))
        sub_measure()
    measure()
